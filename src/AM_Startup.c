...
int main(int argc, char* argv[]){
	...
	memcpy(&servaddr.sin_addr.s_addr, hostinfo->h_addr_list[0], hostinfo->h_length);
    ...
    //Calloc the list that's storing the walls/invalid moves.
    //What sort of datastructure do we want this to be?

	//create a thread for each avatar
	int avatar_id=0;
	pthread_t thread[nAvatars]; //declare as many threads as there are avatars.
	char *ipAd = calloc(16, sizeof(char));

	//convert the IP to readable characters
	inet_ntop(AF_INET, &(servaddr.sin_addr), ipAd, 16); 
	printf("%s\n", ipAd);

	while (avatar_id < nAvatars) {
		printf("%d\n", avatar_id);
		//set up the struct that will pass in the parameters in each thread.
		ThreadParams *tParam = (ThreadParams*)calloc(1, sizeof(ThreadParams));
		tParam->AvatarID = avatar_id;
		tParam->nAvatars = nAvatars;
		tParam->Difficulty = difficulty;
		strcpy(tParam->IP,ipAd);
		tParam->MazePort = recvMessage.init_ok.MazePort;
		strcpy(tParam->FileName, logFileName);
		//create thread
		//You will have to replace PLACEHOLDER with the avatars interacting with the server and getting their positions
		//pthread_create(&thread[avatar_id], NULL, PLACEHOLDER, (void *) tParam);
		avatar_id++;
